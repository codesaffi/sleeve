            ...
            <button
              onClick={async () => {
                if (productData.sizes && productData.sizes.length > 0 && !size) {
                  addToCart(productData._id, undefined); 
                  return;
                }
                await addToCart(
                  productData._id,
              ...
              className="bg-primary hover:bg-black text-background px-8 py-4 font-medium transition-all flex items-center justify-center gap-2 shadow-vintage active:scale-[0.98] w-full sm:flex-1 tracking-wide"
            >
              Order Print
            </button>
          </div>
